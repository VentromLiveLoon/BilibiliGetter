...
def downloader(video_url,audio_url,name):
    ...
    # 保存请求的内容
    with open(f"./videos/{name}_video.mp4",'wb') as file,tqdm(
        desc=name,
        total=total,
        unit='iB',
        unit_scale=True,
    ...
    with open(f"./videos/{name}_audio.mp4",'wb') as file,tqdm(
        desc=name,
        total=total,
        unit='iB',
        unit_scale=True,
    ...
    #开始合并视频和声音
    os.system(f"ffmpeg -i ./videos/{name}_video.mp4 -i ./videos/{name}_audio.mp4 -c copy ./videos/{name}.mp4");
